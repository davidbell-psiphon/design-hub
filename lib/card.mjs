...
export function sessionWire(session) {
  ...
  return {
    ...
    // The agent's last word, moved by every post (piece14). What the board
    // reads as "a run is going" — the only stamp a press or a Stop cannot move.
    agent_seen_at: session.agent_seen_at,
...
export function toWire(card, sessions = []) {
  ...
  return {
    ...
    agent_seen_at: active ? active.agent_seen_at : null,
