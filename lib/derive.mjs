...
// Store-listing screenshot work — flagged, never triggered.
//
// Two signals, either one enough: the `store-screenshots` label, or the word
// "screenshot" in the title. Dave asked for both because the label is new and
// the issues that already exist carry the word instead. What the flag does is
// style the card on the board and relabel its stage buttons for the screenshot
// agent; nothing runs until one of those buttons is pressed, exactly as for
// every other card.
//
// Vendored into design-ai beside deriveBrand, so the board and the runner flag
// the same issues: a card the board shows as Screenshots is a card whose press
// the runner sends to the store-screenshots agent, and never the reverse.
export const SCREENSHOT_LABEL = 'store-screenshots';
export const SCREENSHOT_WORD = /\bscreen ?shots?\b/i;

// From the stored shape — labels as the JSON array the reader writes (or an
// array already parsed, of names or of { name }), title as text. Tolerant of
// malformed JSON, because a board that throws on one bad row renders nothing.
export function isScreenshotWork(labels, title) {
  let names = labels;
  if (typeof names === 'string') {
    try { names = JSON.parse(names); } catch (e) { names = []; }
  }
  if (!Array.isArray(names)) names = [];
  if (names.some(n => (n && typeof n === 'object' ? n.name : n) === SCREENSHOT_LABEL)) return true;
  return SCREENSHOT_WORD.test(String(title || ''));
}

// From a Linear issue as the reader and the runner fetch it.
export function isScreenshotIssue(issue) {
  return isScreenshotWork(
    ((issue && issue.labels && issue.labels.nodes) || []).map(l => l && l.name),
    issue && issue.title,
  );
}
