...
--   npx wrangler deploy
--
-- The session table is called stage_sessions, not sessions. The retired chat
-- organiser still has a `sessions` table in the live database and CREATE TABLE
-- IF NOT EXISTS would have silently done nothing. See piece11-schema.sql.
--
...
-- Read Linear afterwards: Linear owns almost everything in cards, and the read
-- puts it back.
--
...
-- changes made to agent_sessions after the first run. By then the new tables
-- are the truth and the old one is a snapshot.
--
-- NOTHING IS DROPPED. agent_sessions is untouched. It is the rollback.

-- ── Cards ────────────────────────────────────────────────────────────────
--
-- Only rows with a Linear issue. Section 2: a record with no issue key is not a
-- card, so the Hub-only sessions do not become one. They are left in the old
-- table, which is the honest place for them until something needs them again.
--
-- description takes the Linear description, which is what detail held unless an
-- agent had posted to the row. Where an agent had, the description is left null
-- and the next reader pass fills it in from Linear. The context an agent gave
-- goes to sessions.detail below, and the two stop sharing a column for good.
...
--   else research                         the default the reader wrote on every row
--
-- phase can hold qa, which was a stage the board offered and nothing ever
-- implemented (section 12 retired the label). Those land on research. The cost
-- of getting one of them wrong is small: which stage a card has REACHED is
-- derived from Linear labels, never from this row.
--
-- Rows the reader created and nothing ever ran get a session too, and that is
-- deliberate for this one migration: status and prompt on them are real values
-- the board may be rendering right now, and dropping them to make "no session
-- means not started" true from day one would change what is on screen. New
-- cards get no session until a stage is asked for.
...
SELECT
  ...
  response_note,
  responded_at,
  -- Only carried when something is actually queued. requested_at alone is what
  -- says queued now, so carrying it across on a card with no requested_stage
  -- would queue work nobody asked for.
...
--
-- A gate belongs to (issue_key, stage) per section 2, so its history does too.
-- With one session per card there was no ambiguity and session_id alone was
-- enough. With two, a design round archived against the issue would show up in
-- the history of research.
--
-- session_id already holds the issue key after migration-003, so only the stage
-- is missing. It is filled in from the session each card has, which at this
-- point is exactly one.
ALTER TABLE gate_decisions ADD COLUMN stage TEXT;

UPDATE gate_decisions
   SET stage = (SELECT s.stage FROM stage_sessions s WHERE s.issue_key = gate_decisions.session_id)
 WHERE stage IS NULL
   AND EXISTS (SELECT 1 FROM stage_sessions s WHERE s.issue_key = gate_decisions.session_id);

-- ── Check it landed ──────────────────────────────────────────────────────
--
-- Run this after, and read it rather than assuming. The three numbers must
-- match, because the old grain held exactly one session per card:
--
--   SELECT
